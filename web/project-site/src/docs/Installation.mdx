...
Download Jotist from the [Jotist releases page](https://github.com/jaysqvl/Jotist/releases). The first rebrand review release is **v1.7.0-rc.1**. Choose the artifact for your operating system and architecture, and follow the instructions included with that release.
...
Use this configuration for running Jotist on any machine without a dedicated NVIDIA GPU.
...
services:
  scriberr:
    image: ghcr.io/jaysqvl/jotist:1.7.0-rc.1
    ports:
      - "8080:8080"
    volumes:
...
services:
  scriberr:
    image: ghcr.io/jaysqvl/jotist:1.7.0-rc.1-cuda
    ports:
      - "8080:8080"
    volumes:
      - scriberr_data:/app/data # volume for data
