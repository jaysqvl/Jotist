...
Download **Jotist 1.7.2** from the [Jotist releases page](https://github.com/jaysqvl/Jotist/releases/tag/v1.7.2). Choose the artifact for your operating system and architecture, and follow the instructions included with the release.
...
The standard 1.7.2 containers target Linux amd64 with CPU and CUDA 12.6 variants. Blackwell/CUDA 13 remains a local source-build option without actual Blackwell inference qualification and is excluded from the stable release.
...
services:
  scriberr:
    image: ghcr.io/jaysqvl/jotist:1.7.2
    ports:
      - "8080:8080"
    volumes:
...
services:
  scriberr:
    image: ghcr.io/jaysqvl/jotist:1.7.2-cuda
    ports:
      - "8080:8080"
    volumes:
      - scriberr_data:/app/data # volume for data
