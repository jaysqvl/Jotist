...
Download Jotist from the [Jotist releases page](https://github.com/jaysqvl/Jotist/releases). This checkout targets the first stable release, **v1.7.0**; confirm that the desired artifact and image jobs have finished publishing before installation. Choose the artifact for your operating system and architecture, and follow the instructions included with that release.
...
The standard 1.7.0 container publication targets Linux amd64 with CPU and CUDA 12.6 variants. Confirm that both required image jobs have finished on the release page before pulling. Blackwell/CUDA 13 remains a local source-build option without actual Blackwell inference qualification and is excluded from the default stable publication.
