...
func SetupRoutes(handler *Handler, authService *auth.AuthService) *gin.Engine {
	...
	v1 := router.Group("/api/v1")
	{
		...
		transcription.Use(handler.syncBrowserAccessCookie)
		{
			...
			transcription.POST("/:id/rerun", handler.StartTranscription)
			transcription.POST("/:id/kill", handler.KillJob)
			transcription.GET("/:id/queue", handler.ListTranscriptionQueue)
			transcription.POST("/:id/queue", handler.QueueTranscriptionRun)
			transcription.PUT("/:id/queue/order", handler.ReorderTranscriptionQueue)
			transcription.DELETE("/:id/queue/:queue_id", handler.CancelQueuedTranscriptionRun)
			transcription.DELETE("/:id/queue", handler.ClearTranscriptionQueue)
			...
			transcription.GET("/:id/runs", handler.ListJobRuns)
