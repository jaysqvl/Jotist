...
def average_ckpt(ckpt_dir, model, wavlm_only=False, val_metric='Loss', avg_ckpt_num=5, val_mode="best"):
    if os.path.isfile(ckpt_dir):
        ckpt_loaded = torch.load(ckpt_dir, map_location=torch.device('cpu'), weights_only=True)
        if not wavlm_only:
    ...
    if 'checkpoints/epoch_' in ckpt_dir:
        ...
        ckpt_loaded = torch.load(os.path.join(ckpt_dir, 'pytorch_model.bin'), map_location=torch.device('cpu'), weights_only=True)
