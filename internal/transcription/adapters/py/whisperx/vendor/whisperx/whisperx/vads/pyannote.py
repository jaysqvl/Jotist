...
from whisperx.vad_asset import cached_vad_checkpoint
...
def load_vad_model(device, vad_onset=0.500, vad_offset=0.363, token=None, model_fp=None):
    ...
    if model_fp is None:
        # Jotist retains the exact upstream asset without embedding its weights
        # in the server. Cached bytes are verified before deserialization.
        model_fp = cached_vad_checkpoint(model_dir)
    else:
