...
spec = importlib.util.spec_from_file_location(
    "adapter_audit", Path(__file__).with_name("audit-python-adapter-env.py")
)
...
class AuditPolicyTests(unittest.TestCase):
            ...
            "exceptions": [
                {
                    ...
                    "package": "lightning",
                    "version": "2.6.6",
                    "advisory_ids": ["GHSA-known"],
                    "adapters": ["whisperx"],
                    "review_by": "2026-10-13",
                    "reason": "Reviewed specific source fix",
                    "references": ["https://example.invalid/advisory"],
                    "source_sha256": {
                        "saving.py": hashlib.sha256(b"reviewed source\n").hexdigest()
                    },
                }
            ]
        }
    ...
    def match(
        ...
        return audit.matching_exception(
    ...
    def test_changed_materialized_caller_invalidates_unchanged_dependency_exception(
        self,
    ):
    ...
    def test_cuda_versions_query_public_release_without_widening_exception(self):
        ...
        self.assertEqual(
            audit.advisory_version("other", "2.14.0+cu126"), "2.14.0+cu126"
        )
        self.assertEqual(
            audit.advisory_version("torch", "2.14.0+custom"), "2.14.0+custom"
        )
        self.assertIsNone(
            self.match({**self.package, "installed_version": "2.6.6+custom"})
        )

    def test_new_advisory_stays_unreviewed_beside_known_match(self):
        findings = audit.classify_results(
            [self.package],
            [{"vulns": [{"id": "GHSA-known"}, {"id": "GHSA-new"}]}],
            "whisperx",
            self.policy,
            [self.site],
            date(2026, 9, 13),
        ...
        self.assertEqual(
            [f["status"] for f in findings], ["reviewed_exception", "needs_review"]
        )
    ...
    def test_reviewed_local_ports_cannot_hide_upstream_advisories(self):
        ...
        self.assertEqual(
            audit.advisory_version("pyannote.audio", "3.1.1+jotist.1"), "3.1.1"
        )
        self.assertEqual(
            audit.advisory_version("other", "0.0.1+jotist.1"), "0.0.1+jotist.1"
        )
        self.assertEqual(
            audit.advisory_version("diarizen", "0.0.1+jotist.2"), "0.0.1+jotist.2"
        )
